...
from models.results import TrustResult, RiskAssessment, Decision
...
def run_vertical_slice(dataset_path: str, input_csv: str, output_csv: str):
    ...
    for idx, row in messages_df.iterrows():
        ...
        # Drop ground-truth columns if running on sample_messages.csv
        ...
        ctx = RoutingContext(
            ...
            # STUBS for feature services
            trust_result=TrustResult(score=50.0, reasons=["Stub"], signals=[]),
            risk_assessment=RiskAssessment(level="LOW", score=0.0, flags=[], recommendation="safe")
        )
        
        # 3. Stub Decision Engine
        # Heuristic MVP: if group message -> digest, else -> notify
